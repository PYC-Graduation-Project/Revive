...
//
// Skeletal Mesh For Cascade Shadow
//

float4 VSSkeletalMeshForShadowCascade(VS_SKELETAL_MESH_FOR_SHADOW_IN input, uint instance_id : SV_InstanceID) : SV_POSITION
{
    InstanceData i_data = g_instance_data[instance_id];
...
[maxvertexcount(9)]
void GSSkeletalMeshForShadowCascade(triangle float4 input[3] : SV_POSITION, inout TriangleStream<GS_SHADOW_CASCADE_OUT> out_stream)
{
