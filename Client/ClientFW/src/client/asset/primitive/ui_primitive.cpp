...
namespace client_fw
{
	...
	void UIPrimitive::Draw(ID3D12GraphicsCommandList* command_list, UINT lod) const
	{
		command_list->IASetPrimitiveTopology(m_primitive_topology);
		m_vertex_info->Draw(command_list);
		command_list->DrawInstanced(m_num_of_vertex, 1, 0, 0);
	}
}
