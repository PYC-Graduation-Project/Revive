...
namespace revive
{
	...
	void LobbyUILayer::Update(float delta_time)
	{
		if (m_is_wait_matching)
		{
			...
			m_matching_time_text->SetText(L"¸ÅÄª ½Ã°£ [ " + std::to_wstring(static_cast<UINT>(m_matching_time) / 60) +
	...
	bool LobbyUILayer::GenerateMatchingUI(const Vec2& window_size)
	{
		...
		m_user_change_button->OnClicked([this]() {
			...
			if (m_num_of_user == 4)
				m_num_of_user = 2;
			m_num_of_user_text->SetText(L"¸ÅÄª ÀÎ¿ø : " + std::to_wstring(m_num_of_user));
			});
