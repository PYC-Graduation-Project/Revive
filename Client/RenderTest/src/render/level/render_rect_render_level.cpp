...
namespace client_fw
{
	RenderRectRenderLevel::RenderRectRenderLevel(const std::string& name)
		: GraphicsRenderLevel(name, { DXGI_FORMAT_R8G8B8A8_UNORM }, DXGI_FORMAT_D24_UNORM_S8_UINT)
	{
	}

	bool RenderRectRenderLevel::CreateRootSignature(ID3D12Device* device)
	{
	...
	void RenderRectRenderLevel::UpdateCommonResource(ID3D12Device* device, ID3D12GraphicsCommandList* command_list)
	{
	}

	void RenderRectRenderLevel::SetRootCommonResource(ID3D12GraphicsCommandList* command_list)
	{
	}
}
